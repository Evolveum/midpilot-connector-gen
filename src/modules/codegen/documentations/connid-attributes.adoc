...
:page-nav-title: Native to ConnID attribute mapping
...
In this example, we are mapping the native connector attribute `id` to the built-in `UID` attribute in ConnID, and the native connector attribute `name` to the `NAME` attribute in ConnID.

The `connIdAttribute` method takes two parameters: the first is the name of the built-in ConnID attribute (e.g., `UID`, `NAME`), and the second is the exact native attribute name exposed by the connector schema (e.g., `id`, `Username`). For protocol mappings, this native name can differ from the SCIM wire attribute: a native `Username` may map to SCIM `userName`, while the ConnID script must still use `connIdAttribute("NAME", "Username")`. The `connIdAttribute` method accepts only the built-in ConnID attributes as the first parameter.
