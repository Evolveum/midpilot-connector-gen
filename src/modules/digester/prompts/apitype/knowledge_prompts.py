...
get_rest_knowledge_system_prompt = (
    ...
    Your task is to determine, for identity provisioning into a system like midPoint:
    - whether the application is known to provide a REST / OpenAPI (Swagger) HTTP API
      (its OWN custom/proprietary REST API, NOT the SCIM standard), and
    - whether that REST API is generally available or restricted to a paid/enterprise/
      partner plan.

    You MUST produce output that fits the structured schema (RestSignalResult):
    - supportsRest: true ONLY if you are reasonably confident the named application offers
      its own REST/OpenAPI HTTP API usable for provisioning. If you are unsure, or the
      application is unknown to you, return false. A SCIM-only application (no proprietary
      REST API) must return false.
    - availability: one of 'available', 'paid', or 'unknown'. Some products expose their
      REST API only on a paid/enterprise tier or behind a partner/approval program. Use
      'paid' when the REST API is known to require a paid/enterprise/premium/partner plan,
      'available' when it is part of standard/free access, and 'unknown' when you are not
      sure. Default to 'unknown' rather than guessing.
    - requiredPlan: when availability is 'paid' and you know the specific plan/tier name
      (e.g. "Enterprise"), put it here; otherwise leave it empty.
    """).strip()
    + "\n\n"
    + API_TYPE_DEFINITIONS
    + "\n\n"
    ...
    RULES
    - Be conservative and factual. Do NOT guess. If you do not actually know the application or
      are unsure whether it offers a proprietary REST/OpenAPI API, return supportsRest=false.
    - Do not conflate SCIM with REST. A "SCIM REST API" is SCIM, not the product's own REST API;
      it must NOT set supportsRest=true on its own.
    - Do not invent values. Unknown application => supportsRest=false, availability='unknown'.
    - Plan/pricing knowledge is often stale; if you are not confident about tier gating,
      use availability='unknown' and leave requiredPlan empty.
    """).strip()
    + "\n"
)


get_rest_knowledge_user_prompt = textwrap.dedent("""
Application name:
...
From your own knowledge (no documentation is provided), answer:
- Does this application expose its own REST / OpenAPI (Swagger) HTTP API for provisioning to a
  system like midPoint (distinct from the SCIM standard)?
- If it does, is that REST API generally available or restricted to a paid/enterprise/partner
  plan (and which plan)?

Return structured output. If you do not know this application or are unsure, return
supportsRest=false and availability='unknown' rather than guessing.
""")
