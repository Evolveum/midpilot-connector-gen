...
5. *Codegen*: `authorization`, then per object class `native-schema`
   (which also emits the ConnId mapping), `search/{intent}`, `create`,
   `update`, `delete`, and `relations/{relationName}`.
...
6. *Codegen*: per object class `native-schema` (which also emits the ConnId
   mapping), `search/{intent}`, `create`, `update`, `delete`.
   `authorization` has no SQL prompt family; relation code requires a manually
   supplied or otherwise extracted relation.
