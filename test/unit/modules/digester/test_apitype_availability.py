...
def test_paid_outranks_available_when_sources_disagree():
    summary = summarize_scim_availability(
        {
            ScimSource.KNOWLEDGE: ApiTypeSignalResult(supports_scim=True, scim_availability=ScimAvailability.AVAILABLE),
    ...
    assert set(summary.sources) == {ScimSource.KNOWLEDGE, ScimSource.WEB_SEARCH}
...
def test_required_plan_taken_from_paid_signal_only():
    ...
    assert summary.required_plan == ""
    assert summary.sources == (ScimSource.KNOWLEDGE,)


def test_unknown_only_signals_contribute_no_sources():
    summary = summarize_scim_availability(
        {
            ScimSource.KNOWLEDGE: ApiTypeSignalResult(supports_scim=False),
            ScimSource.WEB_SEARCH: ApiTypeSignalResult(supports_scim=False),
        }
    )
    assert summary.status is ScimAvailability.UNKNOWN
    assert summary.sources == ()
