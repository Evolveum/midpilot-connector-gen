...
from src.jobs import _job_futures, schedule_coroutine_job
...
async def test_schedule_coroutine_job_records_session_persistence_failure():
    ...
    with (
        patch("src.jobs.lifecycle.create_job", new_callable=AsyncMock, return_value=job_id),
        patch("src.jobs.lifecycle.set_running", new_callable=AsyncMock),
        patch("src.jobs.lifecycle.set_finished", new_callable=AsyncMock, side_effect=set_finished) as mock_set_finished,
        patch("src.jobs.lifecycle._append_job_error_now", new_callable=AsyncMock) as append_error,
        patch("src.jobs.session_persistence.async_session_maker", MagicMock(return_value=_AsyncSessionContext())),
        patch("src.jobs.session_persistence.SessionRepository", MagicMock(return_value=repo)),
    ):
...
async def test_schedule_coroutine_job_passes_input_payload_to_dynamic_provider_when_declared():
    ...
    with (
        patch("src.jobs.lifecycle.create_job", new_callable=AsyncMock, return_value=job_id),
        patch("src.jobs.lifecycle.update_job_progress", new_callable=AsyncMock),
        patch("src.jobs.lifecycle.set_running", new_callable=AsyncMock),
        patch("src.jobs.lifecycle.set_finished", new_callable=AsyncMock, side_effect=set_finished),
        patch("src.jobs.runner.async_session_maker", MagicMock(return_value=_AsyncSessionContext())),
        patch("src.jobs.runner.JobRepository", MagicMock(return_value=repo)),
        patch("src.jobs.runner.SessionRepository", MagicMock(return_value=session_repo)),
    ):
