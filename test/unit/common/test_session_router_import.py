...
from src.common.errors import InvalidDocumentationImportError
from src.common.session import router
...
async def test_import_documentation_raises_domain_error_before_deleting_existing_chunks(
    ...
    with (
        ...
        patch("src.common.session.router.DocumentationRepository", return_value=doc_repo),
        pytest.raises(InvalidDocumentationImportError) as exc_info,
    ):
        await router.import_documentation_by_id(document, session_id, documentation_id, db=MagicMock())
