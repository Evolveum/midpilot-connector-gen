...
from src.common.auth.keys import generate_api_key, hash_api_key
from src.common.database.config import get_db
from src.common.database.repositories.session_repository import SessionOwner
...
def _patch_auth_repos(active_record=None, session_owner=None):
    ...
    return (
        patch("src.common.auth.dependencies.ApiKeyRepository", return_value=api_key_repo),
        patch("src.common.auth.dependencies.SessionRepository", return_value=session_repo),
    )
...
def test_disabled_mode_allows_requests_without_key(client):
    session_id = uuid4()
    with patch("src.common.session.router.ensure_session_exists", AsyncMock()):
...
def test_management_works_in_disabled_mode_with_master_key(client, master_key_only):
    ...
    with patch("src.common.auth.router.ApiKeyRepository", return_value=api_key_repo):
...
def _get_session_with_key(client, session_owner):
    ...
    with (
        repo_patches[0],
        repo_patches[1],
        patch("src.common.session.router.SessionRepository", return_value=session_repo),
    ):
...
def test_nonexistent_session_passes_ownership_gate(client, enforced_auth):
    ...
    with (
        repo_patches[0],
        repo_patches[1],
        patch("src.common.session.router.ensure_session_exists", AsyncMock()),
    ):
